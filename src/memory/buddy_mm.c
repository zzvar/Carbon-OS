...
struct buddy_mm_struct{
    ...
    atomic_t nr_free[MAX_ORDER + 1];
    page_t *pages_base;
    uint32_t mm_addr_start;
    uint32_t mm_addr_end;
    atomic_t phy_page_count;
    atomic_t phy_page_now_count;
};
...
static uint32_t get_order(uint23_t size)
{
...
static uint32_t buddy_alloc_pages(uint32_t n)
{
    if ((n <= 0) || ((uint32_t)atomic_read(&buddy_mm_info.phy_page_now_count)))
        return 0;
...
static void buddy_free_pages(uint32_t addr,uint32_t n)
{
    ...
    while (n >= order_size)
    {
        ...
        {
            buddy_free_pages_sub(nase,order);
            base += order_size;
            n -= order_size;
        }
        order++;
    ...
    while (n != 0)
    {
        ...
        buddy_free_pages_sub(sub,order);
        base += order_size;
        n -= order_size;
    }
...
static void buddy_show_memory_info(void)
{
    ...
    printk("Physical Memory Pages Info: %d Pages = %d KB\n\n",atomic_read(&buddy_mm_info.phy_page_count),atomic_read(&buddy_mm_info.phy_page_count) * 4);
}
static void buddy_show_management_info(void)
{
    ...
    printk("Physical Memory Pages Used: %d Pages = %d KB\n\n",atomic_read(&buddy_mm_info.phy_page_count) - atomic_read(&buddy_mm_info.phy_page_now_count),(atomic_read(&buddy_mm_info.phy_page_count) - atomic_read(&buddy_mm_info.phy_page_now_count)) * 4);
}
