...
static void ff_page_init(page_t *pages,uint32_t n)
{
    ...
    for (page_t *p = pages;p < pages + n;++p)
    {
        ...
        list_add_before(&p -> list,&f_mm_info.free_list);
    }
    pages[0].ncount = n;
...
static void ff_show_management_info(void)
{
    printk("Physical Memory Pages Used: %d Pages = %d KB\n\n",atomic_read(&ff_mm_info.phy_page_count) - atomic_read(&ff_mm_info.phy_page_now_count),(atomic_read(&ff_mm_info.phy_page_count) - atomic_read(&ff_mm_info.phy_page_now_count)) * 4);
}
